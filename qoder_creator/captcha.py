...
async def solve_slider_local(page, max_attempts: int = 5, console: Console = None, stop_check=None) -> bool:
    """Solve slider Aliyun captcha locally via image processing + mouse drag.

    Returns True jika captcha berhasil diselesaikan.

    ``stop_check`` adalah callback async opsional yang mengembalikan True ketika
    halaman sudah melaju tanpa captcha (misalnya form OTP telah muncul), sehingga
    solver berhenti lebih awal alih-alih mengulang drag yang tidak perlu.
    """
    c = console or Console()

    for attempt in range(1, max_attempts + 1):
        if stop_check is not None and await _stop_requested(stop_check):
            c.print("    [green]captcha sudah tidak diperlukan, lanjut[/]")
            write_log("Captcha solver stopped early: signup already advanced", "SUCCESS")
            return True
...
async def _stop_requested(stop_check) -> bool:
    """Run the optional stop_check without letting page errors break the solver."""
    try:
        return bool(await stop_check())
    except Exception as exc:
        write_log(f"Captcha stop check error: {exc}", "WARNING")
        return False
...
async def _is_solved(page) -> bool:
    ...
    try:
        # Check wrapper visibility â€” jika hidden, captcha solved
        ...
        # Check slider â€” jika hilang setelah solve, captcha solved
        ...
        cls = await page.evaluate(
            ...
            " const ci=c?.querySelector('#aliyunCaptcha-check-icon, [class*=success], [class*=pass]');"
